...
def add_contact_section(pdf):
    change_font(pdf, "")
    add_icon_text(pdf, "./medias/icons/phone.png" , MARGIN + 50 , MARGIN + 43  , PHONE , w=3  , h=3  )
    add_icon_text(pdf, "./medias/icons/email.png" , MARGIN + 100, MARGIN + 43  , EMAIL , w=3.5, h=3  )
    add_icon_text(pdf, "./medias/icons/adress.png", MARGIN + 50 , MARGIN + 49.5, ADRESS, w=3  , h=3.5)



def add_subject(pdf, props):
    pdf.set_xy(MARGIN, MARGIN + 62)
    change_font(pdf, "B")
    pdf.cell(0, LINE_HEIGHT, get_subject(props.get("spontaneous"), props.get("job")))

    pdf.set_xy(MARGIN, MARGIN + 68)
    change_font(pdf, "")
    pdf.cell(0, LINE_HEIGHT, props.get("company"))

    pdf.set_fill_color(89, 88, 96)
    pdf.rect(MARGIN + 1, MARGIN + 78, 160, 0.1, "F")
